...
if(collision(ghost, pacman)){
  ...
  // SPECIAL RULE: force ghost to go UP at row 9
  if (
    ghost.y === tileSize * 9 &&
    ghost.direction !== "U" &&
    ghost.direction !== "D"
...
// remove food AFTER loop
if (foodEaten) {
  foods.delete(foodEaten);
}

// check next level AFTER deletion
if (foods.size === 0) {
  loadMap();
  resetPositions();
}
...
class Ghost extends Block {
  constructor(x, y, width, height, image) {
    super(x, y, width, height); // gives direction & velocity
    this.image = image;
  }
}
...
class Pacman extends Block {
  constructor(x, y, width, height) {
    super(x, y, width, height); // calls Block constructor
